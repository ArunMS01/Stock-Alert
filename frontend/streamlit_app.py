...
tab1, tab2 = st.tabs(["➕ Set New Alert", "📋 My Active Alerts"])
    ...
    if st.button("Add Alert", disabled=not can_submit):
        try:
            response = requests.post(
                ...
                json={"symbol": symbol, "condition": condition, "price": price, "username": username},
            )
            if response.status_code == 200:
                st.success("✅ Alert added successfully!")
                st.info("⏳ Make sure you have messaged the bot so alerts can be delivered.")
            else:
